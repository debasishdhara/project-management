...
<!-- Breadcrumb-->
    ...
    <div class="row">
        <div class="col-lg-12">
            <div class="card">
                <div class="card-body">
                <div class="card-title">City Details</div>
                <hr>
                {!! Form::open(['route'=>['edit-cities',$city_details->id]]) !!}
                <div class="form-group row">
                 <label for="input-26" class="col-sm-2 col-form-label">City Name</label>
                  <div class="col-sm-10">
                    <input type="text" class="form-control {{ $errors->has('city_title') ? 'is-invalid' : '' }}" name="city_title" value="{{ old('city_title', $city_details->title) }}" required>
                    @if ($errors->has('city_title'))
                        <span class="invalid-feedback" role="alert">
                        <strong>{{ $errors->first('city_title') }}</strong>
                    </span>
                    @endif
                  </div>
                </div>
                    ...
                    <div class="col-sm-10">
                        <select class="form-control {{ $errors->has('country_id') ? 'is-invalid' : '' }}" name="country_id" id="country_id" onchange="state_change();" required>
                            ...
                            @foreach($country_all as $country)
                                <option value="{{ $country->id }}" {{ old('country_id', $city_details->state->country_id) == $country->id ? 'selected' : ''}}>{{ $country->title }}</option>
                            @endforeach
                        </select>
                ...
                <div class="form-group row">
                    ...
                    <div class="col-sm-10">
                        <select class="form-control {{ $errors->has('state_id') ? 'is-invalid' : '' }}" name="state_id" id="state_id" required>
                            <option>Select Any</option>
                            @foreach($state_all as $state)
                                <option value="{{ $state->id }}" {{ old('state_id', $city_details->state_id) == $state->id ? 'selected' : ''}}>{{ $state->title }}</option>
                            @endforeach
                        </select>
               ...
               <div class="form-group row">
                 ...
                 <div class="col-sm-10">
                    <input type="checkbox" id="toggle-two" name="city_status" @if(old('city_status',$city_details->city_status)) checked @endif data-onstyle="success" data-offstyle="danger">

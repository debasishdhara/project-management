    ...
    <div class="row">
        <div class="col-lg-12">
            <div class="card">
                ...
                <hr>
                {!! Form::open(['route'=>['edit-permission',$permission_details->id]]) !!}
                <div class="form-group row">
                 <label for="input-26" class="col-sm-2 col-form-label">Permission Name</label>
                  <div class="col-sm-10">
                    <input type="text" class="form-control {{ $errors->has('permission_title') ? 'is-invalid' : '' }}" name="permission_title" value="{{ old('permission_title',$permission_details->permission_name) }}" required>
                ...
                <div class="form-group row">
                    ...
                    <div class="col-sm-10">
                        <input type="checkbox" id="checkbox" @if($role_count==$r_count) checked @endif>Select All
                        <select class="form-control {{ $errors->has('role_id') ? 'is-invalid' : '' }}" name="role_id[]" id="role_id" multiple onchange="checkfall();" required>
                            @foreach($role_all as $role)
                                <option value="{{ $role->id }}" {{ (collect(old('role_id',$role_details))->contains($role->id)) ? 'selected' : ''}}>{{ $role->role_name }}</option>
                            @endforeach
                        </select>
               ...
               <div class="form-group row">
                 ...
                 <div class="col-sm-10">
                    <input type="checkbox" id="toggle-two" name="permission_status" @if(old('permission_status',$permission_details->permission_status)) checked @endif data-onstyle="success" data-offstyle="danger">
